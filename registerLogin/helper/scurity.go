...
func Encrypt2(msg string) string {
	base64Encode := base64.URLEncoding.EncodeToString([]byte(msg))
	hexEncode := hex.EncodeToString([]byte(base64Encode))
	return hexEncode
}

func Decrypt(msg string) string {
	hexDecode, _ := hex.DecodeString(string(msg))
	base64Decode, _ := base64.URLEncoding.DecodeString(string(hexDecode))
	return string(base64Decode)
}

func Decrypt2(msg string) string {
	hexDecode, _ := hex.DecodeString(string(msg))
	base64Decode, _ := base64.URLEncoding.DecodeString(string(hexDecode))
	return string(base64Decode)
}
